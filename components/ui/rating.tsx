...
const ratingVariants = cva('transition-colors', {
  variants: {
    variant: {
      ...
      yellow: 'fill-current text-amber-500 dark:text-amber-400'
    }
  },
  defaultVariants: {
    variant: 'default'
  ...
  const icons = React.useMemo(() => {
    const emptyIcon = React.cloneElement(Icon, {
      ...
      className: cn(
        ...
        variant === 'yellow' &&
          'fill-amber-500/30 stroke-amber-500/10 text-amber-500/10 dark:fill-amber-400/30 dark:stroke-amber-400/10'
      ),
      'aria-hidden': 'true'
    })
